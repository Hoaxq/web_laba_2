...
function createTable(data) {
  ...
  var headerNames = ["Бренд", "Название", "Год создания", "Тираж"];
...
function setSortTable(date, strMass) {
  ...
  for(let i = 0; i < date.length; i++) {
    ...
    if(strMass[1] != "") {
      ...
      if (date[i].Бренд.indexOf(strMass[1]) === -1) {
        bool = false;
      }
    }
    if(strMass[2] != 0) {
      if(date[i]['Год создания'] < strMass[2]) {
        bool = false;
      }
    }
    if(strMass[3] != 0) {
      if(date[i]['Год создания'] > strMass[3]) {
        bool = false;
      }
    }
    if(strMass[4] != 0) {
      if(date[i]['Тираж'] < strMass[4]) {
        bool = false;
      }
    }
    if(strMass[5] != 0) {
      if(date[i]['Тираж'] > strMass[5]) {
        bool = false;
      }
    }
...
allElemMass[0] = '--- Нет ---';
allElemMass[1] = 'Бред';
allElemMass[2] = 'Название';
allElemMass[3] = 'Год создания';
allElemMass[4] = 'Тираж';
